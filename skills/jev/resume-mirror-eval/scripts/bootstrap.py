"""Make `python3 analyze.py` work on a machine with nothing but Python.

The script needs two pure-Python packages: pypdf (PDF text) and openpyxl
...
installs the two packages into it with the pip that ships with Python, and
...
`uv run analyze.py` bypasses all of this: uv resolves the inline metadata
...
REQUIRED = {"pypdf": "pypdf>=4", "openpyxl": "openpyxl>=3.1"}
VENV_DIR = Path(tempfile.gettempdir()) / "resume-mirror-eval" / "venv"
GUARD = "RESUME_MIRROR_EVAL_BOOTSTRAPPED"
...
def _create_venv() -> bool:
    ...
    try:
        ...
        print(f"note: could not create a venv ({err}); continuing without pypdf/openpyxl",
              file=sys.stderr)
        return False


def _install(python: Path) -> bool:
    check = subprocess.run([str(python), "-c", "import pypdf, openpyxl"], capture_output=True)
    if check.returncode == 0:
        return True
    print("installing pypdf and openpyxl into it ...", file=sys.stderr)
    ...
    if result.returncode != 0:
        print(f"note: pip install failed ({result.stderr.strip()[:200]}); "
              "continuing without pypdf/openpyxl", file=sys.stderr)
        return False
    return True
